...
export function SiteFooter() {
  ...
  return (
    <footer className="bg-brand-600 text-white">
      <Container className="grid gap-block py-section-lg lg:grid-cols-[minmax(0,1.4fr)_repeat(3,minmax(0,1fr))] lg:gap-8">
        <div className="max-w-sm">
          <Wordmark tone="inverse" />

          <div className="mt-5 space-y-3 text-body leading-relaxed text-white/80">
        ...
        {Object.entries(siteConfig.footerNav).map(([heading, links]) => (
          <nav key={heading} aria-label={heading}>
            <h2 className="text-h5 font-bold text-white">{heading}</h2>
            <ul className="mt-4 sm:mt-5">
              {links.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="inline-flex min-h-control-sm items-center text-body text-white/80 underline-offset-4 transition-colors hover:text-white hover:underline"
                  >
                    {link.label}
                  </Link>
                </li>
